...
def stream():
    artist = request.args.get('artists')
...
def recommend(trackId):
    ...
    recommendations = getRecommendation(query_vector=query_vector)

    with open("recommended_songs.json", "w") as file:
        json.dump(audio_features, file, indent=2)

    song_data = get_track_metadata(
        [track.get("id") for track in recommendations])
